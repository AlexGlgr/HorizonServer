...
EVENT_MODBUS_LIST = ['modbusclientrot-send', 'modbus-source-toss', 'enqueue-command'];
BUS_NAMES_LIST = ['sysBus', PRIMARY_BUS, 'logBus'];
const PROTOCOL = 'modbusrot';
const THIS_NAME = 'modbusclientrot';
...
class ModbusClientRTUOTCP extends ClassModbusBase_S {
    #_Sources;
    /**
     * @constructor
     * @description
    ...
    constructor({ _busList, _node }) {
        super({ _name: THIS_NAME, _busNameList: BUS_NAMES_LIST, _busList, _node, _type: 'RTUOTCP' });
        this.#_Sources = {};
        this.FillEventOnList('sysBus', EVENT_SYSBUS_LIST);
        this.FillEventOnList(PRIMARY_BUS, EVENT_MODBUS_LIST);
        this.EmitEvents_logger_log({level: 'I', msg: 'MBClient RTU over TCP initialized.'});
    }
     ...
     */
    EmitEvents_proxymodbusrot_msg_get({arg, value}) {
     ...
     * @param {Object} _msg         - само сообщение
     */
    HandlerEvents_modbusclientrot_send(_topic, _msg) {
        try {
     ...
     * @param {String} _conductor   - какая exploit служба будет использовать этот источник
     */
...
module.exports = ModbusClientRTUOTCP;