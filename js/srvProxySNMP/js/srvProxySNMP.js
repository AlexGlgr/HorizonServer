...
class ProxyModbus extends ClassBaseService_S {
    ...
    constructor({ _busList, _primaryBus, _node, _protocol }) {
        ...
        this.FillEventOnList(this.#_PrimaryBus, EVENT_SNMPBUS_LIST);
        this.EmitEvents_logger_log({level: 'I', msg: 'ProxySNMP initialized.'});
    }
     ...
     * @param {*} _msg 
     */
    HandlerEvents_proxysnmp_send(_topic, _msg) {
        const source_name = _msg.metadata.source;
        const source = this.#_SourceMapNames.find(_obj => _obj.Name === source_name);

        if (source != undefined) {
            this.EmitEvents_snmpclient_send({ arg: [_msg.arg, source.chNum], value: [_msg.value[0]]});
        }
    }
    /**
     * @method 
     ...
     */
    HandlerEvents_proxysnmp_msg_get(_topic, _msg) {
     ...
     * @param {*} param0 
     */
    EmitEvents_snmpclient_send({ arg, value }) {
        const msg = {
            dest: 'snmpclient',
            com: 'snmpclient-send',
            arg,
